...
test_that("lda() with LDA.TXT data matches TFQA output with radius 1", {
  ldens <- lda(test_locs$x, test_locs$y, test_locs$type, radius = 1, area = 154)
...
test_that("lda() with LDA.TXT data matches TFQA output with radius 2", {
  ldens <- lda(test_locs$x, test_locs$y, test_locs$type, radius = 2, area = 154)
...
test_that("lda() with LDA.TXT data matches TFQA output with radius 5", {
  ldens <- lda(test_locs$x, test_locs$y, test_locs$type, radius = 5, area = 154)
